...
void sdram_init(void)
{
	...
	if (board_is_eposevm()) {
		...
		///enable_vtt_regulator();    ///adrien- 2020-06-11
		///config_ddr(0, &ioregs_ddr3, NULL, NULL,   ///adrien- 2020-06-12
...
int power_init_board(void)
{
	int rc;
#ifndef CONFIG_DM_I2C
	///struct pmic *p = NULL;  ///adrien- 2020-06-16
#endif
	struct pmic *p = NULL;     ///adrien+ 2020-06-16
	uchar tps_status = 0;      ///adrien+ 2020-06-16
	...
	if (board_is_idk()) {
		 ...
		 * set current limit to 500mA, turn off by default.
		 */
		int ret = 0;
		ret = tps65218_reg_write(TPS65218_PROTECT_L1, TPS65218_CONFIG2, 0x02, TPS65218_CONFIG2_LS2ILIM_MASK); /* 500mA */
		if (ret)
		{
		        printf("Set LS2 current limit to 500mA failed!\n");
			return ret;
		}

		ret = tps65218_reg_write(TPS65218_PROTECT_L1, TPS65218_ENABLE2, 0, TPS65218_ENABLE2_LS2_EN);  /* turn off */
		if (ret)
		{
			printf("Turn off LS2(vbus0) failed!n");
			return ret;
		}
		 ...
		 * set current limit to 500mA, turn on by default.
		 */
		ret = tps65218_reg_write(TPS65218_PROTECT_L1, TPS65218_CONFIG2, 0x02<<2, TPS65218_CONFIG2_LS3ILIM_MASK);   /* 500mA */
		if (ret) 
		{
			printf("Set LS3 current limit to 500mA failed!\n");
			return ret;
		}

		ret = tps65218_reg_write(TPS65218_PROTECT_L1, TPS65218_ENABLE2, TPS65218_ENABLE2_LS3_EN, TPS65218_ENABLE2_LS3_EN);   /* turn on */

		if (ret)
		{
			printf("Turn on LS3(vbus1) failed!\n");
			return ret;
		}
